...
if ! grep -q "def leftover_kinematic_robot" "$REPO/scripts/rsl_rl/beamdojo_runtime.py"; then
  echo "beamdojo_runtime.py is missing leftover robot kinematic drop. Pull ${REF} or catcher cuboid props weld the H1 in place." >&2
  exit 1
fi
if ! grep -q "def leftover_self_collisions_robot" "$REPO/scripts/rsl_rl/beamdojo_runtime.py"; then
