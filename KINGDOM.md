...
3. **Kingdom Research Lab** (`/?tab=research`): syncs `tracking/training-status.json` (gitignored; see the example file) plus proof mp4s. `npm run dev` also polls `/live/training-status.json` every 5s from the BeamDojo checkout (and a fresh W&B heartbeat if `WANDB_API_KEY` is present). The live card shows mean reward / losses / foothold penalty from that snapshot; full curves stay on W&B.
