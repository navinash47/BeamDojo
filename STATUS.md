...
- **Version:** Dual-terrain Stage 1/2 + G1 + catcher/fall path (not yet a 10k-iter train)
- **Agent:** Agent Dojo
- **Progress:** 52%
- **Priority:** P1
...
- Kingdom Research Lab reads `tracking/training-status.json` (gitignored; writer in `beamdojo_runtime.write_training_status`, refreshed every 10 PPO iters). Example: `tracking/training-status.example.json`.
